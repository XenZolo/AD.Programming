...
const UpdateVehicle = () => {
  ...
  const { vehicleId } = location.state || {}; // Get vehicleId from location state
  ...
  useEffect(() => {
    const fetchVehicleDetails = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/vehicles/getById/${vehicleId}` , {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        setVehicleDetails(response.data); // Set the vehicle details in state
      } catch (error) {
...
const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8080/api/vehicles/update/${vehicleId}`, vehicleDetails, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
